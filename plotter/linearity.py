...
def fitOne(filein,selection,erange,suffix):

    xmin,xmax=erange
    nbins = 50
    
    work = ROOT.RooWorkspace()
    work.factory('CBShape::cb(x[{xmin},{xmax}],mean[0,{mmax}],sigma[1,0.8,10],alpha[1,0.1,10],n[5])'.format(xmin=xmin,xmax=xmax,mmax=min(50,xmax)))
    work.factory("a[0,-100,1000]")
    work.factory("b[0,-100,100]")
    work.factory("c[0,-100,100]")
    work.factory("d[0,-100,100]")
    work.factory('Chebychev::bkg(x,{a,b,c,d})')
    
    x = work.var('x')

    var = 'sc_integral*6/8000'

    mark = ROOT.kFullCircle
    line = ROOT.kAzure-6
    ...
    work.factory('nSig[{ns}]'.format(ns=0.5*spectrum.Integral()))
    work.factory('nBkg[{ns}]'.format(ns=0.5*spectrum.Integral()))
    work.factory("SUM::pdfTot(nSig*cb,nBkg*bkg)");
    ...
    # fit spectrum
    rooData.plotOn(frame,ROOT.RooFit.MarkerStyle(mark),ROOT.RooFit.MarkerSize(2))
    pdf = work.pdf('pdfTot')
    pdf.fitTo(rooData,ROOT.RooFit.Save(),ROOT.RooFit.PrintLevel(10))
    pdf.plotOn(frame,ROOT.RooFit.LineColor(line))
    pdf.plotOn(frame,ROOT.RooFit.Components("bkg"),ROOT.RooFit.LineStyle(ROOT.kDotted), ROOT.RooFit.LineColor(ROOT.kCyan))
    pdf.plotOn(frame,ROOT.RooFit.Components("cb"), ROOT.RooFit.LineStyle(ROOT.kDotted), ROOT.RooFit.LineColor(ROOT.kRed))
