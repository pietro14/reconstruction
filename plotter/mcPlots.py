...
legend_ = None;
def doLegend(pmap,mca,corner="TR",textSize=0.035,cutoff=1e-2,cutoffSignals=True,mcStyle="F",legWidth=0.18,legBorder=True,signalPlotScale=None,totalError=None,header="",doWide=False,columns=1,allProcInLegend=False):
        ...
        for p in mca.listSignals(allProcs=True):
            ...
            if p in pmap and pmap[p].Integral() > (cutoff*total if cutoffSignals else 0): 
                ...
                myStyle = mcStyle if type(mcStyle) == str else mcStyle[1] if (len(options.forceFillColorNostackMode) and any(re.match(x,p) for x in options.forceFillColorNostackMode.split(","))) else mcStyle[0]
                ...
                if self._options.printPlots:
                    for ext in self._options.printPlots.split(","):
                        ...
                        if not os.path.exists(fdir): 
                            ...
                            elif os.path.exists("../index.php"): os.system("cp ../index.php "+fdir)
...
def addPlotMakerOptions(parser, addAlsoMCAnalysis=True):
    ...
    parser.add_option("--rspam", dest="rspam",   type="string", default="(LIME - Run2)", help="Spam text on the right hand side");
...
if __name__ == "__main__":
    ...
    if os.path.dirname(outname) and not os.path.exists(os.path.dirname(outname)):
        ...
        elif os.path.exists("../index.php"): os.system("cp ../index.php "+os.path.dirname(outname))
