...
class analysis:
    ...
    def getNEvents(self):
        tf = tf = ROOT.TFile.Open(self.tmpname)
...
if __name__ == '__main__':
    ...
    if not hasattr(options,"pedrun"):
        pf = open("pedestals/pedruns_MC.txt","r")
        peddic = eval(pf.read())
        options.pedrun = -1
        for runrange,ped in peddic.items():
            if int(runrange[0])<=run<=int(runrange[1]):
                ...
                break
        assert options.pedrun>0, ("Didn't find the pedestal corresponding to run ",run," in the pedestals/pedruns_MC.txt. Check the dictionary inside it!")
            
    setattr(options,'pedfile_fullres_name', 'pedestals/pedmap_run%s_rebin1.root' % (options.pedrun))
    
    options.tmpname ="../../Saturation_Mango_Sim/digitization/out/histograms_Run%05d_MC.root" % int(options.run)
